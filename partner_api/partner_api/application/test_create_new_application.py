...
from . import post_create_new_application
...
def set_env():
    env = sys.argv[2]
    print("\nEnv is " + env)
    post_create_new_application.config = partner_api_config.CONF.get(env)
...
def test_post_create_new_application_positive(new_application_json):
    ...
    test_response = post_create_new_application.post_create_new_application(token, new_application_json)
