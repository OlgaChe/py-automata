...
from . import put_update_application_client
...
def set_env():
    env = sys.argv[2]
    print("\nEnv is " + env)
    put_update_application_client.config = partner_api_config.CONF.get(env)


# Update an application
# POST {{baseUrl}}/application
# positive case
@pytest.mark.parametrize('update_application_json', ["partner_api/application/sanity_test/post_create_new_application.json"])
@pytest.mark.sanity
@pytest.mark.positive
@pytest.mark.test_put_update_application_positive_etalon_all_required_data
def test_put_update_application_positive_etalon_all_required_data(update_application_json):
    """
    Positive case: update application with valid data
    """

    # get access token
    token = test_generate_access_token.test_generate_access_token_positive()

    uuid = put_update_application_client.config.get("uuid").get("etalon_uuid_edit")
    ...
    test_response = put_update_application_client.put_update_application(token, json_data, uuid)
